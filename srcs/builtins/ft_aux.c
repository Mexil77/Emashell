...
/*   Updated: 2021/12/31 16:59:02 by emgarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_returnbuiltinvalue(char **exe)
{
	...
	i = 0;
	ret = 0;
	if (!ft_strncmp(exe[0], "cd\0", 3))
		ret = ft_cdretvalue(exe[1]);
	else if (!ft_strncmp(exe[0], "export\0", 7) && ft_splitlen(exe) > 1)
		while (exe[++i] && !ret)
			ret = ft_exportretvalue(exe[i]);
	else if (!ft_strncmp(exe[0], "unset\0", 6))
		while (exe[++i] && !ret)
			ret = ft_unsetbiultinvalue(exe[i]);
	return (ret);
}
